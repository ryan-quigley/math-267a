...
b) The 98% confidence interval for the average time between events, $1/\lambda$, is (5.44, 39.94)  
...
b) The `t.test()` gives a $t$ value of 1.97, which is significantly larger than our critical value, 0.57. Thus, we reject the null hypothesis and conclude that the average weight of loaded logging cars is greater than 140.  
...
d) In light of the new information, the results of the initial t-test are invalid. The data is not a random sample from the population of logging cars, so it is not an appropriate sample to use to test the average weight, $\mu$, of logging cars...