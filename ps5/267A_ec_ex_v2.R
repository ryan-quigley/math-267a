...
## Diagnostic
a <- matrix(idx.mat, ncol = 24, byrow = TRUE)
b <- matrix(1:24, ncol = 3, byrow = TRUE)
dim(b) <- NULL
...
########################################################
# Alternate starting arrangement
...
dim(psgrps.complete)
names(psgrps.complete) <- c(names(psgrps.complete)[1:4], paste("ps", 2:5, sep = ""))
