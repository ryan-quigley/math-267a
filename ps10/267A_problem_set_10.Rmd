...
b) In order to justify performing a t-test, we must assume the underlying population is normally distributed, and we have no reason to believe this assumption is violated. The `t.test()` gives a $t$ value of 1.97, which is significantly larger than our critical value, 0.57. Thus, we reject the null hypothesis and conclude that the average weight of loaded logging cars is greater than 140.  
