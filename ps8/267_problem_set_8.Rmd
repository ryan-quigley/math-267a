...
c) From part (b), $Z \sim \text{N}(0,1) \Rightarrow Y \sim \text{N}(b,a^2)$. Therefore, we have previoulsy shown that plotting the corresponding $p_i$-quantiles of a standard normal distribution agains a normal distribution with mean $b$ and variance $a^2$ gives a straight line with slope that is a function of $a$. Thus, the departure from the 45$^{\circ}$ line in the graph does not indicate that the mystery distribution is not a normal distribution since the points still fall on a straight line; however, it does indicate that the mystery distribution is not standard normal.
...
\intertext{Since $Z$ has the same form of cumulative distribution with $\theta = 1$,}
Q_Z(p) & = -\text{log}(1-p) && \text{for $0 \leq p < 1$} \\
\Rightarrow Q_Z(p) & = \theta Q_Y(p)
\end{align*}
b) From part (a), we have $z_p = \theta y_p$ for $0 \leq p < 1$. There is a linear relationship between $z_p$ and $y_p$, so all points $(y_{p_i}, z_{p_i}), i = 1,...,m$, where $p_i \in [0,1)$ will fall on the straight line with slope = $\theta$ and intercept = 0.  
    ...
    See Figure 1 below for example QQ-plots of the quantiles of $F(a; 2)$ and $F(a; 0.5)$ against the quantiles of $F(a; 1)$
