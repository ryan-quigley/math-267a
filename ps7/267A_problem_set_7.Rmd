...
a) In order to show that $\hat{f}_h(a)$ is a probability density function, we must show two things: (1) $\hat{f}_h(a) \geq 0, ~ \forall a$ and (2) $\int\limits_{-\infty}^{\infty} \hat{f}_h(a) da = 1$.  
    ...
    For condition (1), we know that $n$ is the sample size so it is a positive integer. It is given that $K(u) \geq 0 , ~ \forall u$ and h > 0. This implies that  $\sum\frac{1}{h}K\left(\frac{a - x_i}{h}\right) \geq 0 , ~  \forall a$. Thus, we have shown that $\hat{f}_h(a) \geq 0, ~ \forall a$.  
...
\intertext{From the given information, each of the integrals of the form $\int\limits_{-\infty}^{\infty} K(u)$ evaluates to 1. Additionally, since the kernel is symmetric around 0, the integrals of the form $\int\limits_{-\infty}^{\infty} u K(u) du$ evaluate to 0.}
& = \frac{1}{n} \left[  0 + x_1 \cdot 1 + ... + 0 + x_n \cdot 1  \right] \\
& =  \frac{x_1  + ...  + x_n }{n}   \\
& = \bar{x}
\end{align*}
!!!!INTERPRETATION!!!!
